   ...
   <>
       ...
       <section className="side__bar">
        <div className="side__bar__header">
          ...
          <button className="empty__cart" onClick={empty()}>Esvaziar</button>
        </div>
        <div className="side__bar__content">
        {Object.keys(cart.cart).map((key) => {
          
          return (
            <div key={cart.cart[key].movie.id} className="side__bar__item">
              <img  className="poster_min" src={`https://image.tmdb.org/t/p/w342/${cart.cart[key].movie.poster_path}`} alt=""></img>
              <p className="side__bar__movie__title">{cart.cart[key].movie.original_title}</p>
        ...
        <button className="side__bar__finish"><Link to="/payment">Finalizar compra</Link></button>
    ...
    <section className="cards">
      ...
      return (
        <Card key={movie.id}>
          <CardImage releaseDate={releaseDate} image={`https://image.tmdb.org/t/p/w342/${movie.poster_path}`} alt={movie.original_title} />
          <CardContent>
